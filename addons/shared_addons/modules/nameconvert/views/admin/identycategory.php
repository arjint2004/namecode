...
<section class="item">
	<div class="content">
		...
		<table class="zebra-striped">
			...
			<tr>
				<td><?=$no++?></td>
				<td><?=$datane['kategori']?></td>
				<td><?=$datane['keterangan']?></td>
				<td>
				<?
				if($datane['active']==0){
				?>
					<a href="<?=base_url('admin/nameconvert/activateidentycategory/'.$datane['id'].'/1')?>">Activate</a>
				<?}else{?>
					<a href="<?=base_url('admin/nameconvert/activateidentycategory/'.$datane['id'].'/0')?>">Deactivate</a>
				<? } ?>
				</td>
				<td><a href="<?=base_url('admin/nameconvert/editidentycategory/'.$datane['id'].'')?>">Edit</a></td>
				<td><a href="<?=base_url('admin/nameconvert/deleteidentycategory/'.$datane['id'].'')?>" onclick="if(!confirm('Are you sure?')){return false;}">Delete</a></td>
