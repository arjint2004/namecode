...
class PHPExcel
{
	...
	*/
	...
	function exports($jenis='default',$array=array(),$fileName='export',$header=array(),$save=0){
			...
			$this->getProperties()->setCreator("Studentbook")
										 ->setLastModifiedBy("Asbin Arjinto (arjint2004@gmail.com) studentbook programmer and enginer")
										 ->setTitle("Office 2007 XLSX ".$jenis."")
										 ->setSubject("Office 2007 XLSX  ".$jenis."")
										 ->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.")
										 ->setKeywords("office 2007 openxml php")
										 ->setCategory("Export from database");
			...
			$barish=1;
			$barishx=1;
			foreach($header as $numcoleh=>$dataArrayh){
				$barish++;
				$startcolsh2=$colsh2.$barish;
				foreach($dataArrayh as $dataneh){
					//echo $colsh2++.$barish.' | ';
					$colsbarish=$colsh2++.$barish;
					$this->setActiveSheetIndex(0)->setCellValue($colsbarish, $dataneh);
				}
					...
					$this->getActiveSheet()->getStyle(''.$startcolsh2.':'.$colsbarish.'')->applyFromArray($styleArrayh);
			...
			$cols2='A';
			$startrow=$barishx+2;
			$startcols=$cols.$startrow;
			$startallcols=$cols;
			foreach($array[0] as $numcols=> $dataArraycols){
				//echo $numcols.' | ';
				$this->setActiveSheetIndex(0)->setCellValue($cols.$startrow, $numcols);
				$lastcols=$cols.$startrow;
				$cols++;
				$lastallcols=$cols;
			}
			...
			$this->getActiveSheet()->getStyle(''.$startcols.':'.$lastcols.'')->applyFromArray($styleArray);
			
			
			
			//make auti width column
			foreach(range($startallcols,$lastallcols) as $columnID) {
				$this->getActiveSheet()->getColumnDimension($columnID)
					->setAutoSize(true);
			}
			
			//set cols bold
			$this->getActiveSheet()->getStyle(''.$startcols.':'.$lastcols.'')->getFont()->setBold(true);
			
			//echo $cols.'<br />';
			$baris=$barishx+2;
			$barisx=$startrow+1;
			foreach($array as $numcole=>$dataArray){
				$baris++;
				$startcols2=$cols2.$baris;
				foreach($dataArray as $datane){
					//echo $cols2++.$baris.' | ';
					$colsbaris=$cols2++.$baris;
					$this->setActiveSheetIndex(0)->setCellValue($colsbaris, $datane);
				}
					...
					$this->getActiveSheet()->getStyle(''.$startcols2.':'.$colsbaris.'')->applyFromArray($styleArray);
			...
			if($save==0){
				...
				file_put_contents('/home/studoid1/public_html/depan/uploads/default/files/export_'.$fileName.'.xlsx', $output); 
